...
namespace nsK2Engine {
	...
	/// <param name="size">サイズ。</param>
		...
		/// <param name="radius2">円柱の半径。</param>
		/// <param name="height">円柱の高さ。</param>
		void CreateCylinder(Vector3 pos, Quaternion rot, float radius,float radius2, float height)
		{
			m_physicsGhostObject.CreateCylinder(pos, rot, radius,radius2, height);
		}
		/// <summary>
		...
		const bool IsHit(CharacterControllerCylinder& characterControllerC) const
		{
			bool isCollision = false;
			PhysicsWorld::GetInstance()->ContactTest(characterControllerC, [&](const btCollisionObject& contactObject) {
				if (m_physicsGhostObject.IsSelf(contactObject) == true) {
					isCollision = true;
				}
				});
		...
		const bool IsHit(PhysicsCylinderObject physicsCylinderObject) const
		{
			bool isCollision = false;
			PhysicsWorld::GetInstance()->ContactTest(physicsCylinderObject, [&](const btCollisionObject& contactObject) {
				if (m_physicsGhostObject.IsSelf(contactObject) == true) {
					isCollision = true;
				}
				});
		...
		const bool IsHit(PhysicsSphereObject physicsSphereObject) const
		{
			bool isCollision = false;
			PhysicsWorld::GetInstance()->ContactTest(physicsSphereObject, [&](const btCollisionObject& contactObject) {
				if (m_physicsGhostObject.IsSelf(contactObject) == true) {
					isCollision = true;
				}
				});
