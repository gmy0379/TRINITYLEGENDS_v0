...
Game::Game()
{
	...
	//ゲーム中のBGMを読み込む。
	g_soundEngine->ResistWaveFileBank(8, "Assets/sound/MG_main1.wav");
...
void Game::InitSky()
{
	...
	m_skyCube->SetType((EnSkyCubeType)sky);		//星空のタイプを呼び出し
	m_skyCube->SetPosition(Vector3(-100.0f, -40.0f, -900.0f));
	m_skyCube->SetLuminance(0.4f);
...
void Game::Update()
{
	...
	if (counter->Mcount > 0) {
		if (g_pad[0]->/*IsTrigger*/IsPress(enButtonA))
		{
			...
			/*medal->position.x = (rand() % 300 - 250);
			medal->position.y = ((rand() % 300 - 250) + 400);
			medal->position.z = (-430);*/
			/*medal->firstPosition = medal->position;*/
			counter->Mcount--;
			counter->Mfall++;
		}
	}
