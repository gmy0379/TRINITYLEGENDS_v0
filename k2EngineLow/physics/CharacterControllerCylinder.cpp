...
#include "CharacterControllerCylinder.h"
...
namespace nsK2EngineLow {
	...
	void CharacterControllerCylinder::Init(float radius, float radius2, float height, const Vector3& position)
	{
		m_position = position;
		//コリジョン作成。
		m_radius = radius;
		m_radius = radius2;
		m_height = height;
		m_collider.Init({ radius,radius2, height });
	...
	const Vector3& CharacterControllerCylinder::Execute(Vector3& moveSpeed, float deltaTime)
	{
		if (moveSpeed.y > 0.0f) {
			//吹っ飛び中にする。
			m_isJump = true;
	...
	* @brief	死亡したことを通知。
	*/
	void CharacterControllerCylinder::RemoveRigidBoby()
	{
